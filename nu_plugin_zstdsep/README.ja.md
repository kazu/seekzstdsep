...
### どの nushell に対応するリビジョンか

現在のリビジョンが対象にしているのは **nushell 0.115** です。1 つのビルドが相手にできる nushell は
マイナーバージョン 1 つだけです。`Hello` ハンドシェイクが両側を semver のキャレットで比較し、`0.y.z`
では `y` の一致が要求されるため、0.115 に対してビルドした plugin は 0.114 からも 0.116 からも
拒否されます。

ある nushell に対してどのリビジョンを checkout すればよいかは、`nu_v.<version>` という名前の git tag
に記録してあります。移行は 2 つの tag に挟まれ、移行の両側の commit で切ってあります。`nu_v.0.114` は
0.114 に対してビルドできる最後の commit、`nu_v.0.115` は 0.115 に対してビルドできる最初の commit です。

```sh
git tag -l 'nu_v*'
git checkout nu_v.0.114     # nushell 0.114 と話せる最も新しいリビジョン
```

したがって、後続を持つ tag はその nushell が占める履歴の範囲を上から画します。いちばん新しい tag は
現在の nushell をいつ採用したかを示すだけで、それ以降の作業はブランチの上にあります。
